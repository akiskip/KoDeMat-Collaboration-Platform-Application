...
//    private final HazelcastInstance client;
    ...
    EventList<TelegramField> telegramsList;
...
//    }
    ...
    public TeleEditorTable(VisuHelper helper, long compId) {
        this.helper = helper;
         hazelcastList = helper.getClient().getMap("info_"+compId);
        this.initialize();
...
//       helper.getTelegrams().addItemListener(new TelegramListener(), true);

       
//       create an Eventlist of Telegrams using the hazelcast list
        telegramsList = GlazedLists.threadSafeList(new BasicEventList());
...
//        an event list of elements that are going to be observed
        EventList<TelegramField> observedTelegrams = new ObservableElementList<TelegramField>(telegramsList, telegramConnector);
        ...
        FunctionList<TelegramField, TelegramField> functionList = new FunctionList<TelegramField, TelegramField>(observedTelegrams, forward, reverse);
...
//        rightPanel.setLayout(Layou);
    ...
    public void addEntryToTable(int index, TelegramField tele) {
    ...
    public class TelegramListener implements ItemListener<TelegramField> {
        ...
        public void itemRemoved(ItemEvent<TelegramField> ie) {
            ...
            for (TelegramField telegram : telegramsList) {
                if (telegram.getId() == ie.getItem().getId()) {
                    telegramsList.remove(telegram);
                }
            }
        }
    }
    ...
    public class TelegramEntryListener implements EntryListener<Integer, TelegramField> {
        ...
        public void entryRemoved(EntryEvent<Integer, TelegramField> ee) {
            ...
            telegramsList.remove((int) ee.getKey());
        }
        ...
        public void entryUpdated(EntryEvent<Integer, TelegramField> ee) {
            ...
            telegramsList.set(ee.getKey(), ee.getValue());
